...
/**
 * RenovacaoStatusService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-07-20 10:00
 */

public class RenovacaoStatusService extends CrudService<FichaStatus> {
	...
	@Override
    public Criteria configPagination(Filter<FichaStatus> filter) {
    ...
    @Override
    public void beforeInsert(FichaStatus FichaStatus) {
        if (!FichaStatus.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", FichaStatus.getDescricao()).ne("id", FichaStatus.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(FichaStatus entity) {
    ...
    @Override
    @Admin
    public void remove(FichaStatus car) {
        super.remove(car);
    }
