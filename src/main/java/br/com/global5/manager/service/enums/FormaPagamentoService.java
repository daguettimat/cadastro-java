...
/**
 * FormaPagamentoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class FormaPagamentoService extends CrudService<FormaPagamento> {
	...
	@Override
    public Criteria configPagination(Filter<FormaPagamento> filter) {
    ...
    @Override
    public void beforeInsert(FormaPagamento FormaPagamento) {
        if (!FormaPagamento.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", FormaPagamento.getDescricao()).ne("id", FormaPagamento.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(FormaPagamento entity) {
    ...
    @Override
    @Admin
    public void remove(FormaPagamento car) {
        super.remove(car);
    }
