...
/**
 * TelefoneTipoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class TelefoneTipoService extends CrudService<TelefoneTipo> {
	...
	@Override
    public Criteria configPagination(Filter<TelefoneTipo> filter) {
    ...
    @Override
    public void beforeInsert(TelefoneTipo TelefoneTipo) {
        if (!TelefoneTipo.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", TelefoneTipo.getDescricao()).ne("id", TelefoneTipo.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(TelefoneTipo entity) {
    ...
    @Override
    @Admin
    public void remove(TelefoneTipo car) {
        super.remove(car);
    }
