...
/**
 * FichaClienteService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-04-03 16:08
 */

public class FichaClienteService extends CrudService<FichaCliente> {
	...
	@Override
    public Criteria configPagination(Filter<FichaCliente> filter) {
    ...
    @Override
    public void beforeInsert(FichaCliente fichaCliente) {
        if (!fichaCliente.hasNome()) {
            throw new BusinessException("O nome deve ser preenchido");
        }

        if (crud().eq("nome", fichaCliente.getNome()).ne("id", fichaCliente.getId()).count() > 0) {
            throw new BusinessException("O ID deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(FichaCliente entity) {
    ...
    @Override
    @Admin
    public void remove(FichaCliente fichaCliente) {
        super.remove(fichaCliente);
    }
