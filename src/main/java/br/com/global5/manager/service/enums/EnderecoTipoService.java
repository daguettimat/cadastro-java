...
/**
 * EnderecoTipoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class EnderecoTipoService extends CrudService<EnderecoTipo> {
	...
	@Override
    public Criteria configPagination(Filter<EnderecoTipo> filter) {
    ...
    @Override
    public void beforeInsert(EnderecoTipo EnderecoTipo) {
        if (!EnderecoTipo.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", EnderecoTipo.getDescricao()).ne("id", EnderecoTipo.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(EnderecoTipo entity) {
    ...
    @Override
    @Admin
    public void remove(EnderecoTipo car) {
        super.remove(car);
    }
