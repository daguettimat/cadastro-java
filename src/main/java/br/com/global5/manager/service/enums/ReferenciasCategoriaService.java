...
/**
 * ReferenciasCategoriaService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class ReferenciasCategoriaService extends CrudService<ReferenciasCategoria> {
	...
	@Override
    public Criteria configPagination(Filter<ReferenciasCategoria> filter) {
    ...
    @Override
    public void beforeInsert(ReferenciasCategoria ReferenciasCategoria) {
        if (!ReferenciasCategoria.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", ReferenciasCategoria.getDescricao()).ne("id", ReferenciasCategoria.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(ReferenciasCategoria entity) {
    ...
    @Override
    @Admin
    public void remove(ReferenciasCategoria car) {
        super.remove(car);
    }
