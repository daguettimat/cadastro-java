...
/**
 * ANTTSituacaoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class ANTTService extends CrudService<ANTTSituacao> {
	...
	@Override
    public Criteria configPagination(Filter<ANTTSituacao> filter) {
    ...
    @Override
    public void beforeInsert(ANTTSituacao ANTTSituacao) {
        if (!ANTTSituacao.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", ANTTSituacao.getDescricao()).ne("id", ANTTSituacao.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(ANTTSituacao entity) {
    ...
    @Override
    @Admin
    public void remove(ANTTSituacao car) {
        super.remove(car);
    }
