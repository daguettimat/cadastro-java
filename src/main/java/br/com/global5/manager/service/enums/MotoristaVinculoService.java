...
/**
 * MotoristaVinculoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class MotoristaVinculoService extends CrudService<MotoristaVinculo> {
	...
	@Override
    public Criteria configPagination(Filter<MotoristaVinculo> filter) {
    ...
    @Override
    public void beforeInsert(MotoristaVinculo MotoristaVinculo) {
        if (!MotoristaVinculo.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", MotoristaVinculo.getDescricao()).ne("id", MotoristaVinculo.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(MotoristaVinculo entity) {
    ...
    @Override
    @Admin
    public void remove(MotoristaVinculo car) {
        super.remove(car);
    }
