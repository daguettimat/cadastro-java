...
/**
 * ReboqueTipoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class ReboqueTipoService extends CrudService<ReboqueTipo> {
	...
	@Override
    public Criteria configPagination(Filter<ReboqueTipo> filter) {
    ...
    @Override
    public void beforeInsert(ReboqueTipo ReboqueTipo) {
        if (!ReboqueTipo.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", ReboqueTipo.getDescricao()).ne("id", ReboqueTipo.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(ReboqueTipo entity) {
    ...
    @Override
    @Admin
    public void remove(ReboqueTipo car) {
        super.remove(car);
    }
