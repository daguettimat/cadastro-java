...
/**
 * BairroService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class BairroService extends CrudService<Bairro> {
	...
	@Override
    public Criteria configPagination(Filter<Bairro> filter) {
    ...
    @Override
    public void beforeInsert(Bairro bairro) {
        if (!bairro.hasNome()) {
            throw new BusinessException("O nome do bairro deve ser preenchido");
        }

        if (crud().eq("nome", bairro.getNome()).ne("id", bairro.getId()).count() > 0) {
            throw new BusinessException("O nome do bairro deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(Bairro entity) {
