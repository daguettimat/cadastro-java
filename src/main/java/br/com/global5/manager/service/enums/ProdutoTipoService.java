...
/**
 * ProdutoTipoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class ProdutoTipoService extends CrudService<ProdutoTipo> {
	...
	@Override
    public Criteria configPagination(Filter<ProdutoTipo> filter) {
    ...
    @Override
    public void beforeInsert(ProdutoTipo ProdutoTipo) {
        if (!ProdutoTipo.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", ProdutoTipo.getDescricao()).ne("id", ProdutoTipo.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(ProdutoTipo entity) {
    ...
    @Override
    @Admin
    public void remove(ProdutoTipo car) {
        super.remove(car);
    }
