...
/**
 * ReferenciasAvaliacaoService ( Regras de Negócio )
 * @author zielinski
 * @date 2017-02-21 15:09
 */

public class ReferenciasAvaliacaoService extends CrudService<ReferenciasAvaliacao> {
	...
	@Override
    public Criteria configPagination(Filter<ReferenciasAvaliacao> filter) {
    ...
    @Override
    public void beforeInsert(ReferenciasAvaliacao ReferenciasAvaliacao) {
        if (!ReferenciasAvaliacao.hasDescricao()) {
            throw new BusinessException("A descrição deve ser preenchida");
        }

        if (crud().eq("descricao", ReferenciasAvaliacao.getDescricao()).ne("id", ReferenciasAvaliacao.getId()).count() > 0) {
            throw new BusinessException("O conteúdo da descrição deve ser único");
        }
    }
	
    @Override
    public void beforeUpdate(ReferenciasAvaliacao entity) {
    ...
    @Override
    @Admin
    public void remove(ReferenciasAvaliacao car) {
        super.remove(car);
    }
