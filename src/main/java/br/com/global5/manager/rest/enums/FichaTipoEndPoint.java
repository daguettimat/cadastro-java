...
*
*/
@Path("/enums/ficha/tipo")
@Produces("application/json;charset=utf-8")
public class FichaTipoEndPoint {

	@Inject
    FichaTipoService ficService;
    ...
    public Response create(FichaTipo entity) {
    	ficService.insert(entity);
    	return Response.created(UriBuilder.fromResource(FichaTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	FichaTipo entity = ficService.findById(id);
    	if( entity == null ) {
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<FichaTipo> filter = new Filter<>();
        FichaTipo pes = new FichaTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<FichaTipo> results = ficService.paginate(filter);
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  FichaTipo entity) {
        if (entity == null) {
