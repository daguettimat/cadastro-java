...
*
*/
@Path("/enums/pendencias")
@Produces("application/json;charset=utf-8")
public class PendenciasEndPoint {

	@Inject
    PendenciasService penService;
    ...
    public Response create(Pendencias entity) {
    	penService.insert(entity);
    	return Response.created(UriBuilder.fromResource(PendenciasEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	Pendencias entity = penService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	penService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<Pendencias> filter = new Filter<>();
        Pendencias pes = new Pendencias();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<Pendencias> results = penService.paginate(filter);

       return Response.ok(results).header("count",penService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<Pendencias> filter = new Filter<>();
        Pendencias tc = new Pendencias();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(penService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  Pendencias entity) {
        if (entity == null) {
        ...
        if (penService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            penService.update(entity);
