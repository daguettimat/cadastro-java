...
import br.com.global5.manager.service.enums.SistemaService;
...
*
*/
@Path("/enums/agrupamento/Faturamento")
@Produces("application/json;charset=utf-8")
public class FatAgrupEndPoint {
    ...
    public Response create(Sistema entity) {
    	sisService.insert(entity);
    	return Response.created(UriBuilder.fromResource(FatAgrupEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	Sistema entity = sisService.findById(id);
    	if( entity == null ) {
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<Sistema> filter = new Filter<>();
        Sistema pes = new Sistema();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<Sistema> results = sisService.paginate(filter);
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  Sistema entity) {
        if (entity == null) {
