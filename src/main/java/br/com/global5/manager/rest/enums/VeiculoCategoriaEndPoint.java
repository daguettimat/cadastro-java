...
*
*/
@Path("/enums/veiculo/categoria")
@Produces("application/json;charset=utf-8")
public class VeiculoCategoriaEndPoint {

	@Inject
    VeiculoCategoriaService veiService;
    ...
    public Response create(VeiculoCategoria entity) {
    	veiService.insert(entity);
    	return Response.created(UriBuilder.fromResource(VeiculoCategoriaEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	VeiculoCategoria entity = veiService.findById(id);
    	if( entity == null ) {
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<VeiculoCategoria> filter = new Filter<>();
        VeiculoCategoria pes = new VeiculoCategoria();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<VeiculoCategoria> results = veiService.paginate(filter);
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<VeiculoCategoria> filter = new Filter<>();
        VeiculoCategoria tc = new VeiculoCategoria();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  VeiculoCategoria entity) {
        if (entity == null) {
