...
*
*/
@Path("/pontos/manutencao")
@Produces("application/json;charset=utf-8")
public class ServicosManutencaoEndPoint {
    ...
    public Response create(ServicosManutencao entity) {
    	manService.insert(entity);
    	return Response.created(UriBuilder.fromResource(ServicosManutencaoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) {
    	ServicosManutencao entity = manService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	manService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        ServicosManutencao entity;

        try {
            entity = manService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<ServicosManutencao> filter = new Filter<>();
        ServicosManutencao pes = new ServicosManutencao();
        filter.setEntity(pes);

       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<ServicosManutencao> results = manService.paginate(filter);

       return Response.ok(results).header("count",manService.count(filter)).build();
    }
    ...
    public Response count() {
        Filter<ServicosManutencao> filter = new Filter<>();
        ServicosManutencao tc = new ServicosManutencao();
        filter.setEntity(tc);

        return Response.ok(manService.count(filter)).build();
    }
    ...
    * @status 409 O id passado difere do conteudo encontrado
    * @status 204 Registro alterado com sucesso
    */
    @PUT
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  ServicosManutencao entity) {
        if (entity == null) {
        ...
        if (manService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            manService.update(entity);
