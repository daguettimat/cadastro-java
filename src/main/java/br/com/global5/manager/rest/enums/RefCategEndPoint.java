...
*
*/
@Path("/enums/referencias/categoria")
@Produces("application/json;charset=utf-8")
public class RefCategEndPoint {
    ...
    public Response create(ReferenciasCategoria entity) {
    	refCatService.insert(entity);
    	return Response.created(UriBuilder.fromResource(RefCategEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	ReferenciasCategoria entity = refCatService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	refCatService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        ReferenciasCategoria entity;

        try {
            entity = refCatService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<ReferenciasCategoria> filter = new Filter<>();
        ReferenciasCategoria pes = new ReferenciasCategoria();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<ReferenciasCategoria> results = refCatService.paginate(filter);

       return Response.ok(results).header("count",refCatService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<ReferenciasCategoria> filter = new Filter<>();
        ReferenciasCategoria tc = new ReferenciasCategoria();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(refCatService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  ReferenciasCategoria entity) {
        if (entity == null) {
        ...
        if (refCatService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            refCatService.update(entity);
