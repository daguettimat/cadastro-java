...
import br.com.global5.manager.service.enums.TelefoneOrigemService;
...
*
*/
@Path("/enums/telefone/origem")
@Produces("application/json;charset=utf-8")
public class TelOrigEndPoint {

	@Inject
    TelefoneOrigemService telService;
    ...
    public Response create(TelefoneOrigem entity) {
    	telService.insert(entity);
    	return Response.created(UriBuilder.fromResource(TelOrigEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	TelefoneOrigem entity = telService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	telService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        TelefoneOrigem entity;

        try {
            entity = telService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<TelefoneOrigem> filter = new Filter<>();
        TelefoneOrigem pes = new TelefoneOrigem();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<TelefoneOrigem> results = telService.paginate(filter);

       return Response.ok(results).header("count",telService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<TelefoneOrigem> filter = new Filter<>();
        TelefoneOrigem tc = new TelefoneOrigem();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(telService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  TelefoneOrigem entity) {
        if (entity == null) {
        ...
        if (telService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            telService.update(entity);
