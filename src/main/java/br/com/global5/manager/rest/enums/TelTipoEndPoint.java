...
public class TelTipoEndPoint {

	@Inject
    TelefoneTipoService telService;
    ...
    public Response create(TelefoneTipo entity) {
    	telService.insert(entity);
    	return Response.created(UriBuilder.fromResource(TelTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	TelefoneTipo entity = telService.findById(id);
    	if( entity == null ) {
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<TelefoneTipo> filter = new Filter<>();
        TelefoneTipo pes = new TelefoneTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<TelefoneTipo> results = telService.paginate(filter);
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<TelefoneTipo> filter = new Filter<>();
        TelefoneTipo tc = new TelefoneTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  TelefoneTipo entity) {
        if (entity == null) {
