...
*
*/
@Path("/enums/documento")
@Produces("application/json;charset=utf-8")
public class DocumentoTipoEndPoint {

	@Inject
    DocumentoTipoService docService;
    ...
    public Response create(DocumentoTipo entity) {
    	docService.insert(entity);
    	return Response.created(UriBuilder.fromResource(DocumentoTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	DocumentoTipo entity = docService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	docService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<DocumentoTipo> filter = new Filter<>();
        DocumentoTipo pes = new DocumentoTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<DocumentoTipo> results = docService.paginate(filter);

       return Response.ok(results).header("count",docService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<DocumentoTipo> filter = new Filter<>();
        DocumentoTipo tc = new DocumentoTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(docService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  DocumentoTipo entity) {
        if (entity == null) {
        ...
        if (docService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            docService.update(entity);
