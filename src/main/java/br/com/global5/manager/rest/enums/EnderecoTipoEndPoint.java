...
import br.com.global5.manager.model.enums.EnderecoTipo;
import br.com.global5.manager.service.enums.EnderecoTipoService;
...
public class EnderecoTipoEndPoint {

	@Inject
    EnderecoTipoService endService;
    ...
    public Response create(EnderecoTipo entity) {
    	endService.insert(entity);
    	return Response.created(UriBuilder.fromResource(EnderecoTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	EnderecoTipo entity = endService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	endService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<EnderecoTipo> filter = new Filter<>();
        EnderecoTipo pes = new EnderecoTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<EnderecoTipo> results = endService.paginate(filter);

       return Response.ok(results).header("count",endService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<EnderecoTipo> filter = new Filter<>();
        EnderecoTipo tc = new EnderecoTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(endService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  EnderecoTipo entity) {
        if (entity == null) {
        ...
        if (endService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            endService.update(entity);
