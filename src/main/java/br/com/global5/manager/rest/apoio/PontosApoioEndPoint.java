...
public class PontosApoioEndPoint {

	@Inject
    PontosApoioService ptoService;
    ...
    public Response create(PontosApoio entity) {
    	ptoService.insert(entity);
    	return Response.created(UriBuilder.fromResource(PontosApoioEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	PontosApoio entity = ptoService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	ptoService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<PontosApoio> filter = new Filter<>();
        PontosApoio pes = new PontosApoio();
        filter.setEntity(pes);

       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<PontosApoio> results = ptoService.paginate(filter);

       return Response.ok(results).header("count",ptoService.count(filter)).build();
    }  
    ...
    public Response count() {
        Filter<PontosApoio> filter = new Filter<>();
        PontosApoio tc = new PontosApoio();
        filter.setEntity(tc);

        return Response.ok(ptoService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  PontosApoio entity) {
        if (entity == null) {
        ...
        if (ptoService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            ptoService.update(entity);
