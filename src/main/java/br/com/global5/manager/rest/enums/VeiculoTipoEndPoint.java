...
*
*/
@Path("/enums/veiculo/tipo")
@Produces("application/json;charset=utf-8")
public class VeiculoTipoEndPoint {

	@Inject
    VeiculoTipoService veiTipoService;
    ...
    public Response create(VeiculoTipo entity) {
    	veiTipoService.insert(entity);
    	return Response.created(UriBuilder.fromResource(VeiculoTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	VeiculoTipo entity = veiTipoService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	veiTipoService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        ...
        try {
            entity = veiTipoService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<VeiculoTipo> filter = new Filter<>();
        VeiculoTipo pes = new VeiculoTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<VeiculoTipo> results = veiTipoService.paginate(filter);

       return Response.ok(results).header("count",veiTipoService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<VeiculoTipo> filter = new Filter<>();
        VeiculoTipo tc = new VeiculoTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(veiTipoService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  VeiculoTipo entity) {
        if (entity == null) {
        ...
        if (veiTipoService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            veiTipoService.update(entity);
