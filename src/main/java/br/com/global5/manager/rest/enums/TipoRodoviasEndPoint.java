...
import br.com.global5.manager.service.enums.TipoRodoviasService;
...
*
*/
@Path("/enums/pontos/tipo/rodovias")
@Produces("application/json;charset=utf-8")
public class TipoRodoviasEndPoint {
    ...
    public Response create(TipoRodovias entity) {
    	tprService.insert(entity);
    	return Response.created(UriBuilder.fromResource(TipoRodoviasEndPoint.class)
                .path(String.valueOf(entity.getId())).build()).build();
    }
    
    /**
     * Apagar um registro baseado no ID
     * @param user descricao do usuario habilitado no tpRodovias
     * @param id do tpRodovias
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	TipoRodovias entity = tprService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	tprService.remove(entity);
     ...
     * @param tpRodovias busca na lista por um descricao ou parte dele
     */
    @GET
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
                          @QueryParam("max") @DefaultValue("10") Integer maxResult,
                          @QueryParam("tpRodovias") String tpRodovias) {
        Filter<TipoRodovias> filter = new Filter<>();
        TipoRodovias pes = new TipoRodovias();
        filter.setEntity(pes);
        if(tpRodovias != null){
            filter.getEntity().descricao(tpRodovias);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<TipoRodovias> results = tprService.paginate(filter);

       return Response.ok(results).header("count", tprService.count(filter)).build();
    }  
    ...
    @GET
    @Path("count")
    public Response count(@QueryParam("tpRodovias") String tpRodovias) {
        Filter<TipoRodovias> filter = new Filter<>();
        TipoRodovias tc = new TipoRodovias();
        filter.setEntity(tc);
        if(tpRodovias != null){
            filter.getEntity().descricao(tpRodovias);
        }
        return Response.ok(tprService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  TipoRodovias entity) {
        if (entity == null) {
        ...
        if (tprService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            tprService.update(entity);
