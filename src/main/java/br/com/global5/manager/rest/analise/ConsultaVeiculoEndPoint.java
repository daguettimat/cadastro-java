 ...
 *
 */
@Path("/analise/consulta/veiculo")
@Produces("application/json;charset=utf-8")
public class ConsultaVeiculoEndPoint {

    @Inject
    ConsultaPessoaService pessoaService;
    ...
    public Response create(acConsultaPessoa entity) {
        pessoaService.insert(entity);
        return Response.created(UriBuilder.fromResource(ConsultaVeiculoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) {
        acConsultaPessoa entity = pessoaService.findById(id);
        if (entity == null) {
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<acConsultaPessoa> filter = new Filter<>();
        acConsultaPessoa mercadoria = new acConsultaPessoa();
        filter.setEntity(mercadoria);

        filter.setFirst(startPosition).setPageSize(maxResult);
        final List<acConsultaPessoa> results = pessoaService.paginate(filter);
    ...
    public Response count() {
        Filter<acConsultaPessoa> filter = new Filter<>();
        acConsultaPessoa tc = new acConsultaPessoa();
        filter.setEntity(tc);
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id, acConsultaPessoa entity) {
        if (entity == null) {
