...
*
*/
@Path("/enums/referencias/avaliacao")
@Produces("application/json;charset=utf-8")
public class RefAvalEndPoint {

	@Inject
    ReferenciasAvaliacaoService refAvalService;
    ...
    public Response create(ReferenciasAvaliacao entity) {
    	refAvalService.insert(entity);
    	return Response.created(UriBuilder.fromResource(RefAvalEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	ReferenciasAvaliacao entity = refAvalService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	refAvalService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        ReferenciasAvaliacao entity;

        try {
            entity = refAvalService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<ReferenciasAvaliacao> filter = new Filter<>();
        ReferenciasAvaliacao pes = new ReferenciasAvaliacao();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<ReferenciasAvaliacao> results = refAvalService.paginate(filter);

       return Response.ok(results).header("count",refAvalService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<ReferenciasAvaliacao> filter = new Filter<>();
        ReferenciasAvaliacao tc = new ReferenciasAvaliacao();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(refAvalService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  ReferenciasAvaliacao entity) {
        if (entity == null) {
        ...
        if (refAvalService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            refAvalService.update(entity);
