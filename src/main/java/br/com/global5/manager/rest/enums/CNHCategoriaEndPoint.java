...
public class CNHCategoriaEndPoint {

	@Inject
    CNHCategoriaService cnhService;
    ...
    public Response create(CNHCategoria entity) {
    	cnhService.insert(entity);
    	return Response.created(UriBuilder.fromResource(CNHCategoriaEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    
    /**
     * Apagar um registro baseado no ID
     * @param user descricao do usuario habilitado no sistema
     * @param id da categoria da cnh
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	CNHCategoria entity = cnhService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	cnhService.remove(entity);
     ...
     * @param cnh busca na lista por um descricao ou parte dele
     */
    @GET
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
                          @QueryParam("max") @DefaultValue("10") Integer maxResult,
                          @QueryParam("cnh") String cnh) {
        Filter<CNHCategoria> filter = new Filter<>();
        CNHCategoria pes = new CNHCategoria();
        filter.setEntity(pes);
        if(cnh != null){
            filter.getEntity().descricao(cnh);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<CNHCategoria> results = cnhService.paginate(filter);

       return Response.ok(results).header("count",cnhService.count(filter)).build();
    }  
    ...
    @GET
    @Path("count")
    public Response count(@QueryParam("cnh") String cnh) {
        Filter<CNHCategoria> filter = new Filter<>();
        CNHCategoria tc = new CNHCategoria();
        filter.setEntity(tc);
        if(cnh != null){
            filter.getEntity().descricao(cnh);
        }
        return Response.ok(cnhService.count(filter)).build();
    }    
    ...
    * @status 409 O id passado difere do conteudo identificado no cnh
    * @status 204 Registro alterado com sucesso
    */
    @PUT
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  CNHCategoria entity) {
        if (entity == null) {
        ...
        if (cnhService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            cnhService.update(entity);
