...
import br.com.global5.manager.service.enums.FerramentasSegurancaService;
...
*
*/
@Path("/enums/pontos/ferramentas")
@Produces("application/json;charset=utf-8")
public class FerrSegurancaEndPoint {

	@Inject
    FerramentasSegurancaService fsService;
    ...
    public Response create(FerramentasSeguranca entity) {
    	fsService.insert(entity);
    	return Response.created(UriBuilder.fromResource(FerrSegurancaEndPoint.class)
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	FerramentasSeguranca entity = fsService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	fsService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        FerramentasSeguranca entity;

        try {
            entity = fsService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<FerramentasSeguranca> filter = new Filter<>();
        FerramentasSeguranca pes = new FerramentasSeguranca();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<FerramentasSeguranca> results = fsService.paginate(filter);

       return Response.ok(results).header("count", fsService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<FerramentasSeguranca> filter = new Filter<>();
        FerramentasSeguranca tc = new FerramentasSeguranca();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(fsService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  FerramentasSeguranca entity) {
        if (entity == null) {
        ...
        if (fsService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            fsService.update(entity);
