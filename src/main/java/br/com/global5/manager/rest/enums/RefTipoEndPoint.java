...
import br.com.global5.manager.service.enums.ReferenciasTipoService;
...
*
*/
@Path("/enums/referencias/tipo")
@Produces("application/json;charset=utf-8")
public class RefTipoEndPoint {

	@Inject
    ReferenciasTipoService sisService;
    ...
    public Response create(ReferenciasTipo entity) {
    	sisService.insert(entity);
    	return Response.created(UriBuilder.fromResource(RefTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	ReferenciasTipo entity = sisService.findById(id);
    	if( entity == null ) {
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<ReferenciasTipo> filter = new Filter<>();
        ReferenciasTipo pes = new ReferenciasTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<ReferenciasTipo> results = sisService.paginate(filter);
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<ReferenciasTipo> filter = new Filter<>();
        ReferenciasTipo tc = new ReferenciasTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  ReferenciasTipo entity) {
        if (entity == null) {
