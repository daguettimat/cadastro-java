...
import br.com.global5.manager.model.enums.PerfilFaturamento;
import br.com.global5.manager.service.enums.PerfilFaturamentoService;
...
*
*/
@Path("/enums/perfilfaturamento")
@Produces("application/json;charset=utf-8")
public class PerfilFaturamentoEndPoint {

	@Inject
    PerfilFaturamentoService pfService;
    ...
    public Response create(PerfilFaturamento entity) {
    	pfService.insert(entity);
    	return Response.created(UriBuilder.fromResource(PerfilFaturamentoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	PerfilFaturamento entity = pfService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	pfService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        PerfilFaturamento entity;

        try {
            entity = pfService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<PerfilFaturamento> filter = new Filter<>();
        PerfilFaturamento pes = new PerfilFaturamento();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<PerfilFaturamento> results = pfService.paginate(filter);

       return Response.ok(results).header("count",pfService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<PerfilFaturamento> filter = new Filter<>();
        PerfilFaturamento tc = new PerfilFaturamento();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(pfService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  PerfilFaturamento entity) {
        if (entity == null) {
        ...
        if (pfService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            pfService.update(entity);
