...
import br.com.global5.manager.service.enums.ServicosMotoristaService;
...
*
*/
@Path("/enums/pontos/servicos")
@Produces("application/json;charset=utf-8")
public class ServMotEndPoint {

	@Inject
    ServicosMotoristaService smService;
    ...
    public Response create(ServicosMotorista entity) {
    	smService.insert(entity);
    	return Response.created(UriBuilder.fromResource(ServMotEndPoint.class)
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	ServicosMotorista entity = smService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	smService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
       ...
       final List<ServicosMotorista> results = smService.paginate(filter);

       return Response.ok(results).header("count", smService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        ...
        return Response.ok(smService.count(filter)).build();
    }    
    ...
    */
    ...
    public Response update(@PathParam("id") Long id,  ServicosMotorista entity) {
        ...
        if (smService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            smService.update(entity);
