...
import br.com.global5.manager.model.enums.ReboqueTipo;
import br.com.global5.manager.service.enums.ReboqueTipoService;
...
*
*/
@Path("/enums/reboque/tipo")
@Produces("application/json;charset=utf-8")
public class ReboqueTipoEndPoint {

	@Inject
    ReboqueTipoService rtService;
    ...
    public Response create(ReboqueTipo entity) {
    	rtService.insert(entity);
    	return Response.created(UriBuilder.fromResource(ReboqueTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	ReboqueTipo entity = rtService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	rtService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<ReboqueTipo> filter = new Filter<>();
        ReboqueTipo pes = new ReboqueTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<ReboqueTipo> results = rtService.paginate(filter);

       return Response.ok(results).header("count",rtService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<ReboqueTipo> filter = new Filter<>();
        ReboqueTipo tc = new ReboqueTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(rtService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  ReboqueTipo entity) {
        if (entity == null) {
        ...
        if (rtService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            rtService.update(entity);
