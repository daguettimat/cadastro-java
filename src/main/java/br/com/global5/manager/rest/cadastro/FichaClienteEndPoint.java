...
import br.com.global5.manager.service.cadastro.FichaClienteService;
...
public class FichaClienteEndPoint {

	@Inject
    FichaClienteService fichaService;
    ...
    public Response create(FichaCliente entity) {
    	fichaService.insert(entity);
    	return Response.created(UriBuilder.fromResource(FichaClienteEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	FichaCliente entity = fichaService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	fichaService.remove(entity);
    ...
    public Response findById(@PathParam("id") Long id, @Context Request request,@Context HttpHeaders headers) {
        ...
        try {
            entity = fichaService.findById(id);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<FichaCliente> filter = new Filter<>();
        FichaCliente pes = new FichaCliente();
        filter.setEntity(pes);
        if(nome != null){
            filter.getEntity().nome(nome);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<FichaCliente> results = fichaService.paginate(filter);

       return Response.ok(results).header("count",fichaService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("nome") String nome) {
        Filter<FichaCliente> filter = new Filter<>();
        FichaCliente tc = new FichaCliente();
        filter.setEntity(tc);
        if(nome != null){
            filter.getEntity().nome(nome);
        }
        return Response.ok(fichaService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  FichaCliente entity) {
        if (entity == null) {
        ...
        if (fichaService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            fichaService.update(entity);
