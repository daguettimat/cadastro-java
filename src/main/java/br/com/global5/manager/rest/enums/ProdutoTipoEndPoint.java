...
*
*/
@Path("/enums/produto/tipo")
@Produces("application/json;charset=utf-8")
public class ProdutoTipoEndPoint {

	@Inject
    ProdutoTipoService ptService;
    ...
    public Response create(ProdutoTipo entity) {
    	ptService.insert(entity);
    	return Response.created(UriBuilder.fromResource(ProdutoTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	ProdutoTipo entity = ptService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	ptService.remove(entity);
    ...
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
        ...
        Filter<ProdutoTipo> filter = new Filter<>();
        ProdutoTipo pes = new ProdutoTipo();
        filter.setEntity(pes);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<ProdutoTipo> results = ptService.paginate(filter);

       return Response.ok(results).header("count",ptService.count(filter)).build();
    }  
    ...
    public Response count(@QueryParam("sistema") String sistema) {
        Filter<ProdutoTipo> filter = new Filter<>();
        ProdutoTipo tc = new ProdutoTipo();
        filter.setEntity(tc);
        if(sistema != null){
            filter.getEntity().descricao(sistema);
        }
        return Response.ok(ptService.count(filter)).build();
    }    
    ...
    */
    ...
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  ProdutoTipo entity) {
        if (entity == null) {
        ...
        if (ptService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
