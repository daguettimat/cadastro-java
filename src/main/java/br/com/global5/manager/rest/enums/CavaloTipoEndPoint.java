...
import br.com.global5.manager.service.enums.CavaloTipoService;
...
public class CavaloTipoEndPoint {

	@Inject
    CavaloTipoService cavService;
    ...
    public Response create(CavaloTipo entity) {
    	cavService.insert(entity);
    	return Response.created(UriBuilder.fromResource(CavaloTipoEndPoint.class).path(String.valueOf(entity.getId())).build()).build();
    }
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	CavaloTipo entity = cavService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	cavService.remove(entity);
     ...
     * @param cavalo busca na lista por um descricao ou parte dele
     */
    @GET
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
                          @QueryParam("max") @DefaultValue("10") Integer maxResult,
                          @QueryParam("cavalo") String cavalo) {
        Filter<CavaloTipo> filter = new Filter<>();
        CavaloTipo pes = new CavaloTipo();
        filter.setEntity(pes);
        if(cavalo != null){
            filter.getEntity().descricao(cavalo);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<CavaloTipo> results = cavService.paginate(filter);

       return Response.ok(results).header("count",cavService.count(filter)).build();
    }  
    ...
    @GET
    @Path("count")
    public Response count(@QueryParam("cavalo") String cavalo) {
        Filter<CavaloTipo> filter = new Filter<>();
        CavaloTipo tc = new CavaloTipo();
        filter.setEntity(tc);
        if(cavalo != null){
            filter.getEntity().descricao(cavalo);
        }
        return Response.ok(cavService.count(filter)).build();
    }    
    ...
    * @status 409 O id passado difere do conteudo identificado no cavalo
    * @status 204 Registro alterado com sucesso
    */
    @PUT
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  CavaloTipo entity) {
        if (entity == null) {
        ...
        if (cavService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            cavService.update(entity);
