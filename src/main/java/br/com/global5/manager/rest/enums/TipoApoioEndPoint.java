...
import br.com.global5.manager.service.enums.TipoApoioService;
...
*
*/
@Path("/enums/pontos/tipos")
@Produces("application/json;charset=utf-8")
public class TipoApoioEndPoint {

	@Inject
    TipoApoioService tpService;
    ...
    public Response create(TipoApoio entity) {
    	tpService.insert(entity);
    	return Response.created(UriBuilder.fromResource(TipoApoioEndPoint.class)
                .path(String.valueOf(entity.getId())).build()).build();
    }
    
    /**
     * Apagar um registro baseado no ID
     * @param user descricao do usuario habilitado no tipo
     * @param id do tipo
    ...
    public Response deleteById(@HeaderParam("user") String user, @PathParam("id") Integer id) { 
    	TipoApoio entity = tpService.findById(id);
    	if( entity == null ) {
    		return Response.status(Status.NOT_FOUND).build();
        }
    	tpService.remove(entity);
     ...
     * @param tipo busca na lista por um descricao ou parte dele
     */
    @GET
    public Response list(@QueryParam("start") @DefaultValue("0") Integer startPosition,
                          @QueryParam("max") @DefaultValue("10") Integer maxResult,
                          @QueryParam("tipo") String tipo) {
        Filter<TipoApoio> filter = new Filter<>();
        TipoApoio pes = new TipoApoio();
        filter.setEntity(pes);
        if(tipo != null){
            filter.getEntity().descricao(tipo);
        }
       filter.setFirst(startPosition).setPageSize(maxResult);
       final List<TipoApoio> results = tpService.paginate(filter);

       return Response.ok(results).header("count", tpService.count(filter)).build();
    }  
    ...
    @GET
    @Path("count")
    public Response count(@QueryParam("tipo") String tipo) {
        Filter<TipoApoio> filter = new Filter<>();
        TipoApoio tc = new TipoApoio();
        filter.setEntity(tc);
        if(tipo != null){
            filter.getEntity().descricao(tipo);
        }
        return Response.ok(tpService.count(filter)).build();
    }    
    ...
    * @status 409 O id passado difere do conteudo identificado no tipo
    * @status 204 Registro alterado com sucesso
    */
    @PUT
    @Path("/{id:[0-9][0-9]*}")
    @Consumes("application/json")
    public Response update(@PathParam("id") Long id,  TipoApoio entity) {
        if (entity == null) {
        ...
        if (tpService.crud().eq("id",id).count() == 0) {
            return Response.status(Status.NOT_FOUND).build();
        }
        try {
            tpService.update(entity);
