...
import br.com.global5.infra.util.Assert;

/**
 * @author j r zielinski Helper class to crud an entity
 *
...
public class Crud<T extends BaseEntity> implements Serializable {
    ...
    public Crud<T> fecth(String property, FetchMode value) {
        getCriteria().setFetchMode(property, value);
        return this;
    }
    ...
    public Crud<T> between(String property, java.util.Date dtIni, java.util.Date dtEnd) {
        if (dtIni != null && dtEnd != null) {
            getCriteria().add(Restrictions.between(property, dtIni, dtEnd));
        }  else{
            log.warning("ignoring between restriction due to null value on property "+property);
    ...
    public Session getSession() {
        if (session == null || !session.isOpen()) {
            ...
            //session = HibernateSession.createSessionFactory().openSession();
        } else {
    ...
    public Criteria getCriteria() {
        if (criteria == null || !session.isOpen()) {
    ...
    public List<T> rawQuery(String QueryText) {
        return getEntityManager().createNativeQuery(QueryText).getResultList();
    }
    ...
    public List<T> findByNamedQuery(String namedQueryName, Map<String,String> namedParams) {
